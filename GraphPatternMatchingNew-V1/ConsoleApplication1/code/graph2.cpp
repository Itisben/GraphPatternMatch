...
int Graph::ReadEdges(const char* filename, const char* delim){
	int err = 0;
	int vleft, vright, weight;
	Edge e = { 0 };
	vector<int> data;
	...
	if (0 != error)
	{
		cout<< "open file failed! " << strerror(error) << endl;
		return -1;
	}
	...
	while (fgets(strbuf, 64, fp) != NULL)
	{
		if (0 == strlen(strbuf))
			continue;
		if (strbuf[0] == kIgnoreCurrent)
			continue;
		if (strbuf[0] == kIgnoreAfter)
			break;
		data.clear();
...
int Graph::ReadLabels(const char* filename, const char* delim){
	int err = 0;
	vector<int> data;
	int index, label;
	int error;
...
*/
int Graph::Dfs(DfsFun dfsFun, int argc, void **argv)
{
	stack<Vertex*> stack;
	int time = 0;
	Vertex *u, *v;
...
int Graph::Dijkstra(int s, int direction, DijkstraFun dikstraFun, int argc, void **argv){
	if (false == has_rgraph_ && direction == kIn)
		return -1;
